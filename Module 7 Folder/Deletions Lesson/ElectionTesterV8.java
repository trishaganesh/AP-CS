/**
 * The purpose of this program is to continue our previous program of ArrayLists. We utilize deletion methods for
 * the candidates names and votes, calculate the total votes of all 5 candidates and then, we print/delete various values)
 *
...
public class ElectionTesterV8 {
    ...
    public static void main(String[] args) {
        ...
        changeNameAndVotes(candidate, nametoFind, nametoReplace, changedVotes);
        totalVotes = getTotalVotes(candidate);
        printResults(candidate, totalVotes);
    ...
    public static void deletePosition(ArrayList<Candidate> elections, int position)
    {
    ...
    public static void deleteCandidateName(ArrayList<Candidate> elections, String findName)
    {
        int position = 0;
        int index;
        ...
        for(index = 0; index < elections.size(); index++)
        {
            if(elections.get(index).getName().equals(findName))
            {
                position = index;
                elections.remove(position);
                break;
        ...
        System.out.println();
        System.out.println("<< Delete " +  findName +  " >> ");
    }
